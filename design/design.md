...
# Dashboard (Phase 3)
yagents dashboard [--port 8080]      # 启动 WebUI
```
...
配置文件路径：`~/.config/yagents/config.toml`（可通过 `--config` 覆盖）。

```toml
[daemon]
socket = "~/.local/run/yagents.sock"

[docker]
image = "ubuntu:24.04"
# container = "my-existing-container"  # 可选，复用已有容器而非自动创建

[llm]
provider = "openai"                    # "openai" | "anthropic"
api_key_env = "OPENAI_API_KEY"         # 从环境变量读取 API key
default_model = "gpt-4o"
# base_url = "https://openrouter.ai/api/v1"  # 可选，兼容 OpenAI 的第三方服务

[skills]
paths = ["~/.yagents/skills"]          # Agent Skills 扫描目录

[tavily]
api_key_env = "TAVILY_API_KEY"

# Persona 模板 — 预配置的 agent 角色
[personas.coder]
system_prompt = """You are a senior software engineer. You write clean, well-tested code.
You prefer simple solutions and avoid over-engineering."""
tools = ["execute_bash", "read_file", "write_file", "delete_file", "web_search"]
skills = ["*"]                         # "*" 表示所有已发现的 skills

[personas.researcher]
system_prompt = """You are a research assistant. You search the web thoroughly
...
```
webui/                       # React dashboard (与 src/ 同级)
    package.json
    src/
        ...                  # 复用 @yuulabs/ytrace-ui 组件
```
